"""
Phase 2 pilot: install the Case/CITES schema (legal_schema.gsql) and ingest
the citation-graph pilot slice (every opinion that appears in citations.csv,
i.e. participates in at least one real in-corpus citation -- 12,457 opinions,
9,632 edges, guaranteed no dangling references) into TigerGraph.

Usage:
    python scripts/ingest_pilot_graph.py --install-schema   # first run only
    python scripts/ingest_pilot_graph.py                    # ingest data
"""
import argparse
import csv
import json
...
def main(install_schema_flag: bool):
